...
if ($entite == "medicament") {
    ...
    $medicament_id = $presenteurmed->newMedicament($denomination_medicament, $etat_commercialisation, $damm, $niveau_surveillance, $conditions_prescription, $prix_medicament, $dosages);
    foreach ($substances as $subs) {
        $presenteurmed->newMedicamentSubstance($medicament_id, $subs);
    }
    foreach ($vas as $va) {
        $presenteurmed->newMedicamentVA($medicament_id, $va);
    }
    
    $message= "Medicament ajouté, vous allez être redirigé(e) vers la page d'accueil !";
    ...
    $substance_id = $presenteursubs->newSubstance($denomination_substance, $infos_sup, $classification_atc);
    foreach ($es as $effet) {
        $presenteursubs->newSubstanceES($substance_id, $effet);
    }
    
    $message= "Substance ajoutée, vous allez être redirigé(e) vers la page d'accueil !";
    header('Refresh: 4; URL=http://192.168.100.5/index.php');
    ...
    $medecin_id = $presenteurdoc->newMedecin($nom_medecin, $prenom_medecin, $adresse_cabinet, $adresse_mail);
    foreach ($medicaments as $med) {
        $presenteurdoc->newMedecinMedicament($medecin_id, $med);
    }
    
    $message= "Medecin ajouté, vous allez être redirigé(e) vers la page d'accueil !";
    header('Refresh: 4; URL=http://192.168.100.5/index.php');
...
<html>
    <head>
        <title>Confirmation creation</title>
        <meta charset="UTF-8">
